...
class SequenceDataset_joint_grc(Dataset):
    '''
    ...
    def __init__(self, df, window_size, stride,
        ...
        for data_id in X_copy['id'].unique():
            ...
            fm_sub = df_fm.iloc[X_sub_copy.index]
            ...
            fm_sub = fm_sub.reset_index(drop=True)
            ...
            for j in idxs:
                ...
                prob_1_fm = fm_sub.iloc[j + self.window_size - 1:j + self.window_size].reset_index(drop=True)

                data_tuple = (sample0, label0, sample1, label1, data_id, prob_1_fm)
    ...
    def __getitem__(self, idx):  # get the samples when used rather than load them at once.
        ...
        fm_label = torch.tensor(fm_label.values, dtype=self.dtype)
        fm_label = fm_label.squeeze()

        return sample0, label0, sample1, label1, data_id, fm_label
